...
export function createAquaRunner(scene, runTexture, slideTexture) {
  ...
  const uniforms = {
    ...
    rows: { value: 4 },
    mirror: { value: 0 },
    nextFrame: { value: 0 }, nextMirror: { value: 0 }, mixPose: { value: 0 },
  };
  const material = new THREE.ShaderMaterial({
    uniforms, transparent: true, side: THREE.DoubleSide,
    vertexShader: `varying vec2 vUv;
      void main(){vUv=uv;gl_Position=projectionMatrix*modelViewMatrix*vec4(position,1.);}`,
    fragmentShader: `uniform sampler2D sheet;uniform float frame;uniform float rows;uniform float mirror;uniform float nextFrame;uniform float nextMirror;uniform float mixPose;varying vec2 vUv;
      vec4 pose(float index,float flip){
        ...
        vec2 uv=vec2((mod(index,4.)+clamp(mix(vUv.x,1.-vUv.x,flip),.006,.994))/4.,
          mix(1.-bottom,1.-top,clamp(vUv.y,.012,.988)));
        vec4 c=texture2D(sheet,uv);
        float alpha=c.a*(1.-smoothstep(.10,.34,c.g-max(c.r,c.b)));
        alpha=smoothstep(.45,.85,alpha);
        c.g=min(c.g,max(c.r,c.b)+.025);
        return vec4(c.rgb*alpha,alpha);
      }
      void main(){
        vec4 c=mix(pose(frame,mirror),pose(nextFrame,nextMirror),mixPose);
        if(c.a<.02)discard;
  ...
  function update(dt, { x, jump, slide, menu, moving, speed, laneError, visible = true }) {
    ...
    uniforms.nextFrame.value = running
      ? 8 + skin * 4 + [0,1,2,1][nextStep % 4] : uniforms.frame.value;
    uniforms.nextMirror.value = running && nextStep >= 4 ? 1 : 0;
    // Premultiplied-alpha interpolation creates in-between frames every render,
    // including the seam between the right and left halves of the stride.
    const fraction=phase-Math.floor(phase);
    const transition=THREE.MathUtils.smoothstep(fraction,.25,.75);
    uniforms.mixPose.value=running ? transition : 0;
    const stride=phase*Math.PI/4;
    const bounce = running ? .025 * (1 - Math.cos(stride*2)) : 0;
    root.position.set(x, slide ? .56 : 1.35 + jump + bounce, slide ? .8 : 2);
    root.rotation.set(slide ? -1.15 : 0, 0, slide ? 0 : lean + (running ? Math.sin(stride) * .018 : 0));
    figure.scale.set(slide ? 1.05 : 1, slide ? .95 : 1, 1);
